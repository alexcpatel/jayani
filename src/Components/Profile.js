...
class ProfileAnimation extends React.Component {
  ...
  componentDidUpdate() {
    ...
    console.log(this.picRef.current)
    ...
    return (
      <div className="profile">
        <Transition in={show} timeout={1000}>
          {status => (
            <ProfileAnimation status={status} handleToggle={this.handleToggle} registerCompletedAnimation={this.registerCompletedAnimation} />
          )}
        </Transition>
      </div>
    )
